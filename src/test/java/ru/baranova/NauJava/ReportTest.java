...
public class ReportTest {
    ...
    @Autowired
    ReportTest(FileRepo fileRepo, RoleRepo roleRepo, UserRepo userRepo, FolderRepo folderRepo, FileMetaRepo fileMetaRepo, 
    ...
    // repository test
    @Test
    void emptyReportCreate() {
        Long ids = reportService.createReport();
        Report report = reportRepo.findById(ids).get();
        Assertions.assertEquals(report.getStatus(), ReportStatus.CREATED);
        Assertions.assertEquals(report.getId(), ids);
